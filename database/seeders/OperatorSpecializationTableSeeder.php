...
class OperatorSpecializationTableSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $operator->specializations()->sync([3, 4]);

        $operator = Operator::find(3);
        $operator->specializations()->sync([3, 4]);

        $operator = Operator::find(4);
        $operator->specializations()->sync([4, 5]);

        $operator = Operator::find(5);
        $operator->specializations()->sync([1, 14]);

        $operator = Operator::find(6);
        $operator->specializations()->sync([3, 15]);

        $operator = Operator::find(7);
        $operator->specializations()->sync([11]);

        $operator = Operator::find(8);
        $operator->specializations()->sync([1, 9]);

        $operator = Operator::find(9);
        $operator->specializations()->sync([10, 5]);
        ...
        $operator->specializations()->sync([1]);

        $operator = Operator::find(14);
        $operator->specializations()->sync([2]);

        $operator = Operator::find(15);
        $operator->specializations()->sync([3, 4, 5]);

        $operator = Operator::find(16);
        $operator->specializations()->sync([15, 5]);

        $operator = Operator::find(17);
        $operator->specializations()->sync([2, 6]);

        $operator = Operator::find(18);
        $operator->specializations()->sync([13]);

        $operator = Operator::find(19);
        $operator->specializations()->sync([14]);
